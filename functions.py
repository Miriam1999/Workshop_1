...
def f_descriptive_ob(data_ob: dict) -> dict:
    ...
    data_ob: dict (default: None)
        Entry data of the order book, it is a dictionary with the following structure:
        'timestamp': timestamp object type recognizable by machine, e.g. pd.to_datetime()
        'bid_size': bid levels volume
        'bid': bid levels price
        'ask': ask leves price
    ...
    # -- Weighted-Mid price (p: price, v: volume)
    w_midprice = lambda p, v: (v.iloc[:, 1] / np.sum([v.iloc[:, 0], v.iloc[:, 1]])) * p.iloc[:, 0] \
                              + (v.iloc[:, 0] / np.sum([v.iloc[:, 0], v.iloc[:, 1]])) * p.iloc[:, 1]
    ...
    r_data = {'Median_ts_ob': ob_m1, 'Spread': ob_m2, 'Mid Price': ob_m3, 'Orderbook Imbalance': ob_m8, 
              'wighted-Mid price': ob_m9, 'VWAP': ob_m10}
    return r_data
